...
MPISanitizerDiagnosticInfo::MPISanitizerDiagnosticInfo(const MPISanErrorInfo& Error)
...
void ErrorHandler::reportError(ErrorLevel Level, ErrorCategory Category, StringRef Message) {
  MPISanErrorInfo Error = createErrorInfo(Level, Category, Message);
...
void ErrorHandler::reportMPIError(ErrorLevel Level, StringRef Message, const CallSite& Site) {
  MPISanErrorInfo Error = createErrorInfo(Level, ErrorCategory::CallDetection, Message);
...
void ErrorHandler::reportConfigError(ErrorLevel Level, StringRef Message, StringRef ConfigKey) {
  MPISanErrorInfo Error = createErrorInfo(Level, ErrorCategory::Configuration, Message);
...
void ErrorHandler::reportHookError(ErrorLevel Level, StringRef Message, const Instruction* Inst,
                                   StringRef HookName) {
  MPISanErrorInfo Error = createErrorInfo(Level, ErrorCategory::HookInsertion, Message);
  Error.Inst = Inst;
...
void ErrorHandler::reportAnalysisError(ErrorLevel Level, StringRef Message, const Function* Func,
  ...
  MPISanErrorInfo Error = createErrorInfo(Level, ErrorCategory::StaticAnalysis, Message);
  Error.Func = Func;
...
bool ErrorHandler::shouldContinueAfterError(const MPISanErrorInfo& Error) const {
  RecoveryContext Context = createRecoveryContext(Error, "default");
  return shouldContinueAfterError(Error, Context);
}

RecoveryStrategy ErrorHandler::determineRecoveryStrategy(const MPISanErrorInfo& Error, const RecoveryContext& Context) const {
...
bool ErrorHandler::shouldContinueAfterError(const MPISanErrorInfo& Error, const RecoveryContext& Context) const {
...
bool ErrorHandler::handleUnsupportedPattern(const MPISanErrorInfo& Error, StringRef PatternDescription) {
  ...
  MPISanErrorInfo DegradationInfo(ErrorLevel::Info, ErrorCategory::UnsupportedPattern,
...
void ErrorHandler::collectErrorStatistics(const MPISanErrorInfo& Error) {
  updateStatistics(Error);
...
SmallVector<StringRef, 4> ErrorHandler::generateRecoveryRecommendations(const MPISanErrorInfo& Error) const {
...
SmallVector<const MPISanErrorInfo*, 8> ErrorHandler::getErrorsByCategory(ErrorCategory Category) const {
  SmallVector<const MPISanErrorInfo*, 8> Result;
...
SmallVector<const MPISanErrorInfo*, 8> ErrorHandler::getErrorsByLevel(ErrorLevel Level) const {
  SmallVector<const MPISanErrorInfo*, 8> Result;
  for (const auto& Error : Errors) {
...
std::string ErrorHandler::formatErrorMessage(const MPISanErrorInfo& Error) const {
  std::string Result;
...
void ErrorHandler::reportToDiagnosticEngine(const MPISanErrorInfo& Error) {
...
void ErrorHandler::updateStatistics(const MPISanErrorInfo& Error) {
  // Update level counts
  switch (Error.Level) {
    case ErrorLevel::Info:
...
MPISanErrorInfo ErrorHandler::createErrorInfo(ErrorLevel Level, ErrorCategory Category, StringRef Message) {
  MPISanErrorInfo Error(Level, Category, Message);
...
RecoveryContext ErrorHandler::createRecoveryContext(const MPISanErrorInfo& Error, StringRef Phase) const {
...
bool ErrorHandler::isErrorSevereInContext(const MPISanErrorInfo& Error, const RecoveryContext& Context) const {
  // Error is severe if:
